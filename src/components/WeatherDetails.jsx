...
export default function WeatherDetails({ weather, unit, themeMode }) {
  ...
  const formatTime = (timestamp) => {
    const date = new Date(
      (timestamp + (weather.timezone - 25200)) * 1000
    );
      ...
      {detailsList.map((detail) => (
        <div
          ...
          className={`
            ...
            border border-solid transition-all duration-500
        ...
        >
          {/* Glow effect */}
          <div
            className={`absolute -right-8 -bottom-8 w-24 h-24 rounded-full blur-2xl pointer-events-none opacity-0 group-hover:opacity-10 transition-opacity duration-500 ${
              detail.accent.includes('blue')
                ? 'bg-blue-500'
                : detail.accent.includes('sky')
                ? 'bg-sky-500'
                : detail.accent.includes('purple')
                ? 'bg-purple-500'
                : detail.accent.includes('teal')
                ? 'bg-teal-500'
                : detail.accent.includes('amber')
                ? 'bg-amber-500'
                : 'bg-orange-500'
            }`}
          />
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div
              className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-transform duration-500 group-hover:scale-110 shrink-0 ${detail.accent}`}
            >
              {detail.icon}
            </div>
          </div>
